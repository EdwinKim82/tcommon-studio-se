...
public class ModuleAccessHelper {
    ...
    private static boolean skipIllegalAccessCheck(Property property) {
        ...
        if (isJava17) {
            String skip = System.getProperty(JavaUtils.SKIP_ILLEGAL_ACCESS_CHECK);
            if (skip != null) {
            ...
            ProjectPreferenceManager preferenceManager = new ProjectPreferenceManager(project, CoreRuntimePlugin.PLUGIN_ID,
                    false);
            return preferenceManager.getBoolean(JavaUtils.SKIP_ILLEGAL_ACCESS_CHECK);
        }
        return false;
    }
    ...
    public static Set<String> getModuleAccessVMArgsForProcessor(IProcessor processor) {
        ...
        if (property == null || property.getItem() == null || !skipIllegalAccessCheck(property)) {
            return Collections.emptySet();
        }
